...
@endif
@section('content')
    <x-page-header eyebrow="Cuti" title="Cuti" description="Kelola data cuti karyawan." icon="bi-diagram-3-fill">
        <x-slot:badges>
            <x-badge>{{ $cuti->total() }} cuti</x-badge>
        </x-slot:badges>

        <x-slot:actions>
            <x-button variant="outline" icon="bi-trash" href="{{ route('cuti.trash') }}">
                Trash
            </x-button>

            <x-button variant="primary" icon="bi-plus-lg" data-bs-toggle="offcanvas" data-bs-target="#offcanvas-cuti">
                Tambah Cuti
            </x-button>
        </x-slot:actions>
    </x-page-header>
    ...
    <x-panel>
        ...
        <div id="cuti-table">
                            ...
                            <td class="app-table__col-actions">
                                <div class="app-table__actions">
                                    ...
                                    <x-button variant="icon-edit" icon="bi-pencil" data-bs-toggle="offcanvas"
                                        data-bs-target="#offcanvas-cuti-edit"
                                        data-edit-url="{{ url('cuti/' . $row['id'] . '/edit-data') }}"
                                        data-update-url="{{ route('cuti.update', $row['id']) }}" />
                                    ...
                                    <form action="{{ route('cuti.destroy', $row->id) }}" method="POST">
                                        ...
                                        <x-button type="submit" variant="icon-danger" icon="bi-trash"
                                            onclick="return confirm('Hapus cuti ini?')" />
                                    </form>
            ...
            <div class="app-table-footer">
                <span>
                    Menampilkan
                    {{ $cuti->firstItem() ?? 0 }}–{{ $cuti->lastItem() ?? 0 }}
                    dari
                    {{ $cuti->total() }}
                    entri
                </span>

                <x-pagination :paginator="$cuti" />
            </div>
        </div>
    </x-panel>

    @include('cuti.form-create')
    @include('cuti.form-edit')
    <script>
